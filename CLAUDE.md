...
- [x] Go SDK — `clients/go/` module (`github.com/devcairn/polargraph-go`); `Client` struct with functional options `WithAPIKey`, `WithTLSCA`; full method surface; 5 unit tests; `clients/go/README.md`
...
- [x] SPARQL type-testing filter built-ins — `isLiteral(?x)`, `isIRI(?x)`, `isURI(?x)` (synonym → `IsIri`), `isBlank(?x)` (always false — PolarGraph has no blank nodes); `SparqlFilter::IsLiteral` and `SparqlFilter::IsBlank` variants added; `Function::IsLiteral` / `Function::IsBlank` wired in `translate_filter`; evaluated in `apply_sparql_filter`; 14 tests in `crates/polargraph-sparql/tests/sparql.rs`
