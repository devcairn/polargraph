...
fn test_cardinality_one_to_many() {
    ...
    assert!(reg.validate_cardinality("manages", subject, object1, &store).is_ok());

    // Commit the first triple.
    store.insert(&relation(subject, "manages", object1)).unwrap();
    ...
    assert!(result.unwrap_err().message.contains("one object per subject"));

    // Different subject → still ok.
    let other_subject = NodeId::new();
    assert!(reg.validate_cardinality("manages", other_subject, object1, &store).is_ok());
}
...
fn test_cardinality_many_to_one() {
    ...
    // First insert: ok.
    assert!(reg.validate_cardinality("owned_by", subject1, object, &store).is_ok());

    store.insert(&relation(subject1, "owned_by", object)).unwrap();
    ...
    assert!(result.is_err());
    assert!(result.unwrap_err().message.contains("one subject per object"));

    // Different object → ok.
    let other_object = NodeId::new();
    assert!(reg.validate_cardinality("owned_by", subject2, other_object, &store).is_ok());
}
...
fn test_cardinality_one_to_one() {
    ...
    // First insert ok.
    assert!(reg.validate_cardinality("paired_with", a, b, &store).is_ok());
    store.insert(&relation(a, "paired_with", b)).unwrap();

    // Same subject, different object → subject already has one.
    assert!(reg.validate_cardinality("paired_with", a, c, &store).is_err());
    // Different subject, same object → object already has one.
    assert!(reg.validate_cardinality("paired_with", c, b, &store).is_err());
}
...
fn test_cardinality_many_always_ok() {
    ...
    reg.register_edge_type(
        EdgeTypeDef::new("knows", None::<String>, None::<String>, vec![]),
    ).unwrap();
...
fn test_inverse_of_lookup() {
    ...
    reg.register_edge_type(
        EdgeTypeDef::new("sibling_of", None::<String>, None::<String>, vec![]),
    ).unwrap();

    assert_eq!(reg.inverse_predicate("parent_of"), Some("child_of".to_string()));
...
fn test_inverse_of_persists_across_reopen() {
    ...
    assert_eq!(reg2.inverse_predicate("follows"), Some("followed_by".to_string()));
}
...
fn test_subtype_resolved_fields() {
    ...
    reg.register_type(NodeTypeDef::new("Animal", vec![
        FieldDef::required("name", FieldKind::Text),
        FieldDef::optional("weight", FieldKind::Float),
    ])).unwrap();

    reg.register_type(
        NodeTypeDef::new("Dog", vec![
            FieldDef::required("breed", FieldKind::Text),
        ])
...
fn test_subtype_resolved_fields_own_field_wins() {
    ...
    reg.register_type(NodeTypeDef::new("Base", vec![
        FieldDef::optional("x", FieldKind::Int),
    ])).unwrap();

    // Child declares 'x' as required — own declaration should win.
    reg.register_type(
        NodeTypeDef::new("Child", vec![
    ...
    assert!(x_field.required, "child's required declaration should shadow parent's optional");
...
fn test_subtype_is_subtype_of() {
    ...
    reg.register_type(
        NodeTypeDef::new("B", vec![]).with_parent_types(vec!["A"]),
    ).unwrap();
    reg.register_type(
        NodeTypeDef::new("C", vec![]).with_parent_types(vec!["B"]),
    ).unwrap();
...
fn test_subtype_validate_inherits_required_fields() {
    ...
    reg.register_type(NodeTypeDef::new("Vehicle", vec![
        FieldDef::required("make", FieldKind::Text),
        FieldDef::required("year", FieldKind::Int),
    ])).unwrap();

    reg.register_type(
        NodeTypeDef::new("Car", vec![
            FieldDef::required("doors", FieldKind::Int),
        ])
        .with_parent_types(vec!["Vehicle"]),
    ).unwrap();

    // Missing inherited fields → error.
    let errs = reg.validate_properties("Car", &HashMap::from([
    ...
    let result = reg.validate_properties("Car", &HashMap::from([
        ("doors".to_string(), polargraph_core::value::Value::Int(4)),
        ("make".to_string(), polargraph_core::value::Value::Text("Toyota".into())),
...
fn test_subtype_cycle_rejected() {
    ...
    reg.register_type(
        NodeTypeDef::new("Y", vec![]).with_parent_types(vec!["X"]),
    ).unwrap();

    // Z extends Y, but Y extends X and X has no parents — ok so far.
    reg.register_type(
        NodeTypeDef::new("Z", vec![]).with_parent_types(vec!["Y"]),
    ).unwrap();

    // Now try to make X extend Z — this would create X→Y→Z→X cycle.
    let result = reg.register_type(
        NodeTypeDef::new("X", vec![]).with_parent_types(vec!["Z"]),
    );
    assert!(result.is_err(), "expected cycle detection error");
    let err_msg = result.unwrap_err().to_string();
    assert!(err_msg.to_lowercase().contains("cycle"), "error should mention cycle: {err_msg}");
}
...
fn test_self_cycle_rejected() {
    ...
    let result = reg.register_type(
        NodeTypeDef::new("Loop", vec![]).with_parent_types(vec!["Loop"]),
    );
    assert!(result.is_err());
}
